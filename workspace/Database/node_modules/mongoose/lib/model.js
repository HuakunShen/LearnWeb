 ...
 * [`mongoose.models()`](./api.html#mongoose_Mongoose-models) and
 * [`connection.models()`](./api.html#connection_Connection-models) functions
 * create subclasses of `mongoose.Model` as shown below.
 *
 * ####Example:
 *
 *     // `UserModel` is a "Model", a subclass of `mongoose.Model`.
 *     const UserModel = mongoose.models('User', new Schema({ name: String }));
 *
 *     // You can use a Model to create new documents using `new`:
 *     const userDoc = new UserModel({ name: 'Foo' });
 *     await userDoc.save();
 *
 *     // You also use a models to create queries:
...
function Model(doc, fields, skipId) {
  if (fields instanceof Schema) {
    throw new TypeError('2nd argument to `Model` must be a POJO or string, ' +
      '**not** a schema. Make sure you\'re calling `mongoose.models()`, not ' +
      '`mongoose.Model()`.');
  }
...
/**
 * If this is a discriminator models, `baseModelName` is the name of
 * the base models.
 *
 * @api public
 ...
 *
 * @api public
 * @fires error whenever any query or models function errors
 * @memberOf Model
 * @static events
 */
...
/*!
 * Compiled middleware for this models. Set in `applyHooks()`.
 *
 * @api private
 ...
 *
 *     var doc = new Tank;
 *     doc.models('User').findById(id, callback);
 *
 ...
 *
 *     var Person = mongoose.models('Person', PersonSchema);
 ...
 *
 * @param {String} name discriminator models name
 * @param {Schema} schema discriminator models schema
 ...
 *
 * Mongoose calls this function automatically when a models is created using
 * [`mongoose.models()`](/docs/api.html#mongoose_Mongoose-models) or
 * * [`connection.models()`](/docs/api.html#connection_Connection-models), so you
 ...
 *     var Event = mongoose.models('Event', eventSchema);
 *
...
/**
 * Create the collection for this models. By default, if no indexes are specified,
 * mongoose will not create the collection for the models until any documents are
...
/**
 * Makes the indexes in MongoDB match the indexes defined in this models's
 * schema. This function will drop any indexes that are not defined in
 * the models's schema except the `_id` index, and build any indexes that
 ...
 *     const Customer = mongoose.models('Customer', schema);
...
function _decorateDiscriminatorIndexOptions(model, indexOptions) {
  // If the models is a discriminator and it has a unique index, add a
...
/*!
 * Connection instance the models uses.
 *
 * @property db
 * @api public
 * @memberOf Model
...
/**
 * Base Mongoose instance the models uses.
 *
 * @property base
 * @api public
 * @memberOf Model
...
/**
 * Registered discriminators for this models.
 *
...
Model.findByIdAndUpdate = function(id, update, options, callback) {
  ...
  // if a models is passed in instead of an id
 ...
 * ####Note:
 *
 * Be careful to not use an existing models instance for the update clause (this won't work and can cause weird behavior like infinite loops). Also, ensure that the update clause does not have an _id property, which causes Mongo to return a "Mod on _id not allowed" error.
 *
 * ####Note:
 *
 ...
 *     o.verbose = true;
 *
 *     User.mapReduce(o, function (err, models, stats) {
 *       console.log('map reduce took %d ms', stats.processtime)
 *       models.find().where('value').gt(10).exec(function (err, docs) {
 *         console.log(docs);
 *       });
 *     })
 *
 ...
 *       var stats = res.stats;
 *       console.log('map reduce took %d ms', stats.processtime)
 *       return models.find().where('value').gt(10).exec();
 *     }).then(function (docs) {
 *        console.log(docs);
 ...
 * ####NOTE:
 *
 * - Arguments are not cast to the models's schema because `$project` operators allow redefining the "shape" of the documents at any stage of the pipeline, which may leave documents in an incompatible format.
 ...
 * - models: optional name of the models to use for population
 ...
 *         { path: 'notes', options: { limit: 10 }, models: 'override' }
 *       ];
 *
 ...
 *     })
 *
 *     // imagine a Weapon models exists with two saved documents:
 *     //   { _id: 389, name: 'whip' }
 ...
 *     Weapon.populate(user, { path: 'weapon', models: 'Weapon' }, function (err, user) {
 ...
 *       });
 *     });
 *     // Note that we didn't need to specify the Weapon models because
 ...
 * Populate helper
 *
 * @param {Model} models the models to use
 * @param {Document|Array} docs Either a single document or array of documents to populate.
 * @param {Object} paths
  ...
  function _getModelNames(doc, schema) {
    ...
    if (modelNameFromQuery) {
      ...
      if ((ref = get(schemaForCurrentDoc, 'options.ref')) != null) {
        ...
        // populate using the base models by default
...
Model.compile = function compile(name, schema, collectionName, connection, base) {
  ...
  if (typeof name === 'function' && name.prototype instanceof Model) {
    ...
    model = function model(doc, fields, skipId) {
      ...
      // Otherwise, just use the top-level models
...
/*!
 * Register custom query methods for this models
 *
 * @param {Model} models
 * @param {Schema} schema
 */
...
/*!
 * Subclass this models with `conn`, `schema`, and `collection` settings.
 *
...
Model.__subclass = function subclass(conn, schema, collection) {
  // subclass models using this connection and collection name
  const _this = this;
