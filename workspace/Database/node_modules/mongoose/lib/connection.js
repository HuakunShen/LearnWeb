 ...
 *     db.plugins.length; // 1
 *
 *     db.models('Test', new Schema({})); // Prints "Applied"
 *
 * @property plugins
 * @memberOf Connection
 * @instance
...
Connection.prototype.dropDatabase = _wrapConnHelper(function dropDatabase(cb) {
  // If `dropDatabase()` is called, this models's collection will not be
 ...
 *
 * Not typically needed by applications. Just talk to your collection through your models.
 *
...
/**
 * Declares a plugin executed on all schemas you pass to `conn.models()`
 *
...
/**
 * Defines or retrieves a models.
 *
 *     var mongoose = require('mongoose');
 *     var db = mongoose.createConnection(..);
 *     db.models('Venue', new Schema(..));
 *     var Ticket = db.models('Ticket', new Schema(..));
 *     var Venue = db.models('Venue');
 *
 * _When no `collection` argument is passed, Mongoose produces a collection name by passing the models `name` to the [utils.toCollectionName](#utils_exports.toCollectionName) method. This method pluralizes the name. If you don't like this behavior, either pass a collection name or set your schemas collection name option._
 *
 * ####Example:
 *
 ...
 *     var M = conn.models('Actor', schema, collectionName)
 *
 * @param {String|Function} name the models name or class extending Model
 * @param {Schema} [schema] a schema. necessary when defining a models
 * @param {String} [collection] name of mongodb collection (optional) if not given it will be induced from models name
 * @see Mongoose#model #index_Mongoose-models
 * @return {Model} The compiled models
 * @api public
 */

Connection.prototype.model = function(name, schema, collection) {
  if (!(this instanceof Connection)) {
    throw new MongooseError('`connection.models()` should not be run with ' +
      '`new`. If you are doing `new db.models(foo)(bar)`, use ' +
      '`db.models(foo)(bar)` instead');
  }
  ...
  if (this.models[name] && !collection) {
    // models exists but we are not subclassing with custom collection
  ...
  if (schema && schema.instanceOfSchema) {
    ...
    // only the first models with this name is cached to allow
  ...
  if (this.models[name] && collection) {
    // subclassing current models with alternate collection
  ...
  // lookup models in mongoose module
  ...
  if (this === model.prototype.db
    ...
    // models already uses this connection.
...
/**
 * Removes the models named `name` from this connection, if it exists. You can
 * use this function to clean up any models you created in your tests to
 * prevent OverwriteModelErrors.
 *
 * ####Example:
 *
 *     conn.models('User', new Schema({ name: String }));
 *     console.log(conn.models('User')); // Model object
 *     conn.deleteModel('User');
 *     console.log(conn.models('User')); // undefined
 *
 *     // Usually useful in a Mocha `afterEach()` hook
 *     afterEach(function() {
 *       conn.deleteModel(/.+/); // Delete every models
 *     });
 *
 * @api public
 * @param {String|RegExp} name if string, the name of the models to remove. If regexp, removes all models whose name matches the regexp.
 * @return {Connection} this
 */
...
/**
 * Returns an array of models names created on this connection.
 * @api public
 * @return {Array}
 */
